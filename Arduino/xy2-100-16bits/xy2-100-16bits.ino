...
// Protocol Constants
#define PACKET_LENGTH   19     // 20 bits per frame (3 control + 16 data + 1 unused)
#define DATA_MIDPOINT   32768L // Center value for 16-bit data (2^15)
#define CONTROL_BITS    0x1    // 001 binary (shifted to bits 19-17)

// INDEPENDENT CHANNEL SETTINGS
#define DATA_AMPLITUDE_X  32767L  // X channel amplitude (50% of 32768)
#define DATA_AMPLITUDE_Y  32767L  // Y channel amplitude (50% of 32768)
...
void setup() {
    ...
    Serial.print("Control Bits: 001 (binary)");
...
void loop() {
    ...
    if (currentTime - lastSampleTime >= SAMPLE_DELAY_US) {
        ...
        // Build the packet for 16-bit mode with 3 control bits:
        // Format: [C2=0] [C1=0] [C0=1] [D15] [D14] ... [D0] [Unused]
        // Shift control bits to bits 19-17, data to bits 16-1
        uint32_t xPacket = (CONTROL_BITS << 17) | ((xData & 0xFFFF) << 1);
        uint32_t yPacket = (CONTROL_BITS << 17) | ((yData & 0xFFFF) << 1);
        ...
        if (xCurrentIndex % 100 == 0) {
            ...
            Serial.print("X Packet: 0x"); Serial.print(xPacket, HEX);
            Serial.print(" Y Packet: 0x"); Serial.println(yPacket, HEX);
        }
    }
}
